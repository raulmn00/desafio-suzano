...
export interface DefinirAgendamentoInput {
  id: string;
  dataEntrega: Date;
  janelaInicio: string;
  janelaFim: string;
...
export class DefinirAgendamentoUseCase {
  constructor(
  ...
  async executar(input: DefinirAgendamentoInput): Promise<OrdemVendaView> {
    ...
    await this.transactionManager.executar(async () => {
      ...
      await this.auditLogger.registrar({
        ator: input.ator,
        acao: AcaoAuditoria.ORDEM_VENDA_AGENDAMENTO_DEFINIDO,
